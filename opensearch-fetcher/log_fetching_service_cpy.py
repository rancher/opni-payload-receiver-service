    ...
    while True:
        ...
        if "_scroll_id" in current_page:
            ...
            if len(current_page_results) > 0:
                ...
                try:
                    last_fetched_results = await es.search(index="last_fetched_timestamp", body={"query": {"match_all": {}}})
                    logging.info(last_fetched_results)
                except Exception as e:
                    logging.error("Unable to get value.")
                break
        else:
            break
...
async def fetch_logs():
    ...
    try:
        ...
        last_fetched_timestamp_exists = await es.indices.exists("last_fetched_timestamp")
        if not last_fetched_timestamp_exists:
            index_created = await es.indices.create(index="last_fetched_timestamp", body={"mappings": {"properties": {"last_fetched_ts": {"type": "integer"}}}})
            await es.index(index="last_fetched_timestamp",body={"last_fetched_ts": current_ts})
        else:
            await es.indices.delete(index='last_fetched_timestamp', ignore=[400, 404])
            index_created = await es.indices.create(index="last_fetched_timestamp", body={"mappings": {"properties": {"last_fetched_ts": {"type": "integer"}}}})
            await es.index(index="last_fetched_timestamp",body={"last_fetched_ts": current_ts})
            last_fetched_results = await es.search(index="last_fetched_timestamp", body={"query": {"match_all": {}}})
            logging.info("here are the last_fetched_results")
            logging.info(last_fetched_results)
        current_ts = int((datetime.now().timestamp() - TIME_RANGE_SECONDS) * 1000)
        #await send_all_results_to_nats(last_fetched_timestamp, current_ts, True)
        while True:
